...
void MainWindow::credit_calc() {
  std::pair<QStringList, double> ansDiff =
      ControlNode::getControlNode().diffCredit(
          ui->sum->value(), ui->duration->value(), ui->percent->value());

  double annPay = ControlNode::getControlNode().annCredit(
      ui->sum->value(), ui->percent->value(), ui->duration->value());

  ui->back_sum->setText(QString::number(annPay * ui->duration->value()));
  ui->overkill->setText(
      QString::number(annPay * ui->duration->value() - ui->sum->value()));
...
void MainWindow::eval() {
  if (ControlNode::getControlNode().is_valid()) {
    ...
    evaBuilder =
        evaBuilder.ToReversePolish(ui->mainLabel->text().toStdString());
    if (ui->tabWidget->currentIndex() == 0) {
      evaBuilder =
          evaBuilder.PlotEnable(ui->x_min->value(), ui->y_min->value(),
                                ui->x_max->value(), ui->y_max->value());
    }
    Stage answer = evaBuilder.Calculate(ui->doubleSpinBox->value());
    ui->answerLabel->setText(QString::number(answer.result));
    ui->plot->graph(0)->setData(QVector<double>(answer.x.begin(),answer.x.end()),
                                QVector<double>(answer.y.begin(),answer.y.end()));
